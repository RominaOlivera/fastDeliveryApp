...
class IncreaseCartItemUseCaseTest {

    private val repository: CartRepository = mockk(relaxed = true)
    private lateinit var useCase: IncreaseCartItemUseCase

    @Before
    fun setup() {
        useCase = IncreaseCartItemUseCase(repository)
    }

    @Test
    fun `invoke should call repository increaseQuantity`() = runTest {
        val item = CartItemEntity("1", "Test", 10.0, 1, "image")
        useCase(item)
        coVerify { repository.increaseQuantity(item) }
    }
}
