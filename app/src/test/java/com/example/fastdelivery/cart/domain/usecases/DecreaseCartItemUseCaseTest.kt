...
class DecreaseCartItemUseCaseTest {

    private val repository: CartRepository = mockk(relaxed = true)
    private lateinit var useCase: DecreaseCartItemUseCase

    @Before
    fun setup() {
        useCase = DecreaseCartItemUseCase(repository)
    }

    @Test
    fun `invoke should call repository decreaseQuantity`() = runTest {
        val item = CartItemEntity("1", "Test", 10.0, 1, "image")
        useCase(item)
        coVerify { repository.decreaseQuantity(item) }
    }
}
